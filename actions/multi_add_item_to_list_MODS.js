module.exports = {
  ...
  subtitle(data, presets) {
    return `Добавлять ${data.branches.length == 1 ? data.branches.length + " Элемент" : data.branches.length + " Элементов"}`;
  },
...
<dialog-list id="branches" fields='["value", "valueeval", "mode", "position", "positionvalue"]' dialogResizable dialogTitle="Item" dialogWidth="600" dialogHeight="400" listLabel="Itens" listStyle="height: calc(100vh - 300px);" itemName="Item" itemHeight="28px;" itemTextFunction="glob.formatItem(data)" itemStyle="line-height: 28px;">
    <div style="padding-top:20px;margin-right:10px;margin-left:10px;" onmouseover="(function(){
      ...
      <select id="mode" class="round" onchange="(function(){
        ...
        <option value="1">Выражение</option>
      </select>
      ...
      <select id="position" class="round" onchange="(function(){
        ...
        <option value="0" selected>Добавить в конце</option>
        <option value="1">Добавить в начале</option>
        <option value="3">Добавить в случайную позицию</option>
        <option value="2">Добавить в определенную позицию</option>
      </select>
...
`;
  ...
  init() {
    ...
    glob.formatItem = function (data) {
      ...
      switch(position) {
        case 0:
          result += "Добавить к концу: ";
          break;
        case 1:
          result += "Добавить к началу: ";
          break;
        case 2:
          result += `Добавить в положение ${data.positionvalue}: `;
          break;
        case 3:
          result += "Случайное добавление: ";
          break;
      }
