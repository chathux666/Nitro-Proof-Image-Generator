module.exports = {
  ...
  subtitle(data, presets) {
    return `Получить ${data.branches.length == 1 ? data.branches.length + " вешь" : data.branches.length + " элементов"}`;
  },
...
<dialog-list id="branches" fields='["info", "position", "storage", "varName", "storage2", "varName2"]' dialogResizable dialogTitle="Item" dialogWidth="600" dialogHeight="400" listLabel="Itens" listStyle="height: calc(100vh - 300px);" itemName="Item" itemHeight="28px;" itemTextFunction="glob.formatItem(data)" itemStyle="line-height: 28px;">
    <div style="padding-top:20px;margin-right:10px;margin-left:10px;" onmouseover="(function(){
        ...
        if(avalue == '6') {
          ...
          document.getElementById('span').innerHTML = 'Выберите из x в x';
        } else if(avalue == '2') {
          document.getElementById('divValue').style.display = 'block';
          document.getElementById('span').innerHTML = 'Позиция';
        } else {
      ...
      <span class="dbminputlabel">Выташить</span><br>
      <select id="info" class="round" onchange="(function(){
        ...
        <option value="7" selected>Первый пункт</option>
        <option value="0">Средний элемент</option>
        <option value="1">Последний пункт</option>
        <option value="2">Позиция</option>
        <option value="3">Случайный предмет</option>
        <option value="4">Случайный элемент [пара]</option>
        <option value="5">Случайный элемент [ODD]</option>
        <option value="6">Случайный элемент [выберите]</option>
      </select>

      <br>

      <div id="divValue">
        <span id="span" class="dbminputlabel">Позиция</span>
      ...
      <div style="float: left; width: 35%;">
        <span class="dbminputlabel">Хранить позицию</span><br>
    ...
    <div style="float: left; width: 35%;">
      <span class="dbminputlabel">Хранить информацию о элемент</span><br>
...
`;
  ...
  init() {
    ...
    glob.formatItem = function (data) {
      let result = '<div style="display: inline-block; width: 200px; padding-left: 8px;">Выташить: ';
      const info = parseInt(data.info);

      switch(info) {
        case 0:
          result += "Средний элемент ";
          break;
        case 1:
          result += "Последний пункт";
          break;
        case 2:
          result += `Позиция ${data.position}`;
          break;
        case 3:
          result += "Случайный элемент";
          break;
        case 4:
          result += "Случайный элемент [пара]";
          break;
        case 5:
          result += "Случайный элемент [ODD]";
          break;
        case 6:
          result += "Случайный элемент [выберите]";
          break;
        case 7:
          result += "Первый пункт";
          break;
