module.exports = {
  ...
  subtitle(data, presets) {
    return `Проверять ${data.branches.length} возможности`;
  },
...
<tab-system>
  <tab label="Информация" icon="align left">
    <span class="dbminputlabel">Информация</span>
    <textarea class="round" id="storage" rows="16" placeholder="Информация..."></textarea>
  </tab>

  <tab label="Сравнения и действия" icon="list alternate">
    <dialog-list id="branches" fields='["comparison", "value", "value2", "actions"]' dialogResizable dialogTitle="Check Variable Info" dialogWidth="600" dialogHeight="400" listLabel="Сравнения и действия" listStyle="height: calc(100vh - 250px);" itemName="Condition" itemHeight="28px;" itemTextFunction="glob.formatItem(data)" itemStyle="line-height: 28px;">
  ...
  action(cache) {
    ...
    const variable = this.evalMessage(data.storage, cache);
    let result = false;
    if (variable) {
      const val1 = variable;
